...
public class CppImpl implements Impls{
    @Override
    public String saveProblem(Question question, String path) {
        String targetFolder = path + "/src/" + question.getGroup(), fileName = question.getName() + ".cpp";
    ...
    public String getQuestionDatas(String currentFilePath) {
        return currentFilePath.replace("src", ".cci_questions").replace(".cpp", ".json");
    }
}
